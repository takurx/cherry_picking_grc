...
class satnogs_noaa_apt_recoder(gr.top_block):
    ...
    def __init__(self, antenna=satnogs.not_set_antenna, bb_gain=satnogs.not_set_rx_bb_gain, decoded_data_file_path='/tmp/.satnogs/data/noaa', dev_args='rtl=00000001', doppler_correction_per_sec=20, enable_iq_dump=0, flip_images=0, if_gain=satnogs.not_set_rx_if_gain, iq_file_path='/tmp/iq.dat', lo_offset=100e3, ogg_file_path='/tmp/test.ogg', ppm=0, rf_gain=satnogs.not_set_rx_rf_gain, rigctl_port=4532, rx_freq=90.4e6, rx_sdr_device='rtlsdr', sync=1, waterfall_file_path='/tmp/waterfall.dat', wav_file_path='/tmp/test.wav'):
        ...
        self.satnogs_ogg_encoder_0 = satnogs.ogg_encoder(ogg_file_path, 48000, 0.8)
        ...
        self.blocks_wavfile_sink_0 = blocks.wavfile_sink(wav_file_path, 1, 48000, 8)
    ...
    def get_ogg_file_path(self):
        return self.ogg_file_path

    def set_ogg_file_path(self, ogg_file_path):
        self.ogg_file_path = ogg_file_path
...
def main(top_block_cls=satnogs_noaa_apt_recoder, options=None):
    if options is None:
        options, _ = argument_parser().parse_args()

    tb = top_block_cls(antenna=options.antenna, bb_gain=options.bb_gain, decoded_data_file_path=options.decoded_data_file_path, dev_args=options.dev_args, doppler_correction_per_sec=options.doppler_correction_per_sec, enable_iq_dump=options.enable_iq_dump, flip_images=options.flip_images, if_gain=options.if_gain, iq_file_path=options.iq_file_path, lo_offset=options.lo_offset, ogg_file_path=options.ogg_file_path, ppm=options.ppm, rf_gain=options.rf_gain, rigctl_port=options.rigctl_port, rx_freq=options.rx_freq, rx_sdr_device=options.rx_sdr_device, sync=options.sync, waterfall_file_path=options.waterfall_file_path, wav_file_path=options.wav_file_path)
    tb.start()
    tb.wait()
